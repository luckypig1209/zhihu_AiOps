  ...
  options: {
    ...
    drawEdge: {
      ...
      start: function (event) {
        ...
        // 检查起始节点是否为文字节点类型，如果是则不允许开始绘制线条
        if (sourceNodeModel.type === "cc-text") {
          // 不执行后续操作
          return;
        }
        ...
        if (self.drawEdge.isMoving) {
          if (self.drawEdge.currentLine === event.item) {
            ...
            if (targetNode) {
              ...
              // 检查目标节点是否为文字节点类型，如果是则不允许连接
              if (targetNodeModel.type === "cc-text") {
                // 移除当前绘制的线条
              ...
              // 线条创建完成后，调用addDeleteButton方法添加删除按钮
              if (self.drawEdge.currentLine && base.addDeleteButton) {
                // 直接调用base中定义的addDeleteButton方法
                base.addDeleteButton(self.drawEdge.currentLine);
              }
