...
const service = axios.create({
  ...
  baseURL: '/admin-api/', // 此处的 /admin-api/ 地址，原因是后端的基础路径为 /admin-api/
  // baseURL: '/admin-api/',
...
service.interceptors.request.use(config => {
  ...
  config.headers['Accept-Language'] = localStorage.getItem('accept-language') ? localStorage.getItem('accept-language') : 'en-US';
...
service.interceptors.response.use(async res => {
  ...
  if (ignoreMsgs.indexOf(msg) !== -1) { // 如果是忽略的错误码，直接返回 msg 异常
    ...
    if (msg === i18n.t('common.invalidRefreshToken')) { // hard coding：忽略这个提示，直接登出
      ...
      if (requestUrl.includes('/asset-info/import')) {
        Message({
          message: msg.replace(/\n/g, "<br>"),
          type: 'error',
          dangerouslyUseHTMLString: true,
...
export function getBaseHeader() {
  return {
    ...
    'Accept-Language': localStorage.getItem('accept-language') || 'en-US',
  }
}
