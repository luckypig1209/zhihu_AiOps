<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="用户编号" prop="userId" :rules="[{pattern: /^[0-9]*$/,message: '请输入数值形式的用户编号', trigger: 'blur'}]">
        <el-input v-model="queryParams.userId" placeholder="请输入用户编号" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="用户名称" prop="userType">
        <el-input v-model="queryParams.userType" placeholder="请输入用户名称" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
  ...
  methods: {
    ...
    getList() {
      ...
      let para = {
        ...
        userName:this.queryParams.userType,
        templateCode:this.queryParams.templateCode,
        templateType:this.queryParams.templateType,
        createStartTime:this.queryParams.createTime[0],
        createEndTime:this.queryParams.createTime[1]
      }
      getNotifyMessagePage(para).then(response => {
