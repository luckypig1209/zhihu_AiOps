 ...
 */
<template>
   <Card style="border-radius: 24px;height: 52vh;">
    <div>
      <div style="margin-right: 8px" class="itemTitle">
          <span>数据统计</span>
          <el-select v-model="selectValue" style="width: 200px;" @change="getInfoChange(selectValue)">
            <el-option value="7days" label="近一周每日数据趋势"></el-option>
            <el-option value="30days" label="近一月每日数据趋势"></el-option>
            <el-option value="365days" label="近一年每日数据趋势"></el-option>
          </el-select>
      </div>
    <div class="row" style="margin-top: 20px;">
      <Card style="border-radius: 12px">
        <div class="con">
        <div class="item" style="height: 16vh;width: 48%;">
          <div class="img"><img :src="require('@/assets/images/icon01.png')" /></div>
          <div class="txt">
            <p style="color: #f5525c" @click="openModal(0, '告警', detail.alarmCount)">{{ detail.alarmCount }}</p>
            <h3>告警</h3>
          </div>
          <div>
            <barChart :id="'ssss'" :color="'rgb(245, 82, 92)'" :dataArray="alarmArray"></barChart>
          </div>
        </div>
        <div class="item" style="height: 16vh;width: 48%;">
          <div class="img"><img :src="require('@/assets/images/icon02.png')" /></div>
          <div class="txt">
            <p style="color: #26bef3" @click="openModal(1, '工单', detail.workOrderCount)">
              {{ detail.workOrderCount }}
            </p>
            <h3>所有工单</h3>
          </div>
            <div>
            <barChart :id="'aaaa'" :color="'rgb(38, 190, 243)'" :dataArray="processArray"></barChart>
          </div>
        </div>
      </div>
      </Card>
       <Card style="margin-top: 10px;border-radius: 12px">
     <div class="con">
        <div class="item" style="height: 16vh;width: 48%;">
          <div class="img"><img :src="require('@/assets/images/icon03.png')" /></div>
          <div class="txt">
            <p style="color: #4f5bff" @click="openModal(2, '隐患', detail.hiddenDangerIdentificationCount)">
              {{ detail.hiddenDangerIdentificationCount }}
            </p>
            <h3>隐患</h3>
          </div>
            <div>
            <barChart :id="'eeeee'" :color="'rgb(79, 91, 255)'" :dataArray="dagerArray"></barChart>
          </div>
        </div>
      ...
      <!-- <Card class="left-card">
        ...
        <div class="item" style="height: 16vh;width: 48%;">
          <div class="img"><img :src="require('@/assets/images/icon04.png')" /></div>
          <div class="txt">
            <p style="color: #ff903a" @click="openModal(3, '服务计划', detail.projectCount)">
              {{ detail.projectCount }}
            </p>
            <h3>服务计划</h3>
          </div>
            <div>
            <barChart :id="'gggg'" :color="'rgb(255, 144, 58)'" :dataArray="planArray"></barChart>
          </div>
        </div>
...
import { getFourCount, getAlarm, getErrorSheet, getSchedule, getHiddenTrouble, getAlarmChange, getDangerChange, getProcessInChange,  getPlanChange, getAllSheet} from "@/api/login";
...
export default {
  ...
  methods: {
    ...
    getDetail() {
      ...
      getAllSheet()
        .then((res) => {
          // 所有工单
          if (res.code === 0) {
            this.detail.workOrderCount = res.data.total || 0;
          } else {
    ...
    openModal(index, title, value) {
      ...
      if(menus.some(item=>item.path ==toMenus[index])){
        ...
        const paramArr = [null, null, "status", "activeName"];
        const valueArr = [null, null, "2", "todo"];
  ...
  created() {
    this.getDetail();
    this.getInfoChange(this.selectValue)
    // this.getAlarmCount();
    this.getGjCount();
